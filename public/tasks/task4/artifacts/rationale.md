...
For this task the initial run is a strong failure candidate, scoring 24 / 87. The model opened both distractor screenshots and correctly recognized neither maps to a logged bug, and its issue-key thread came out fully correct: it searched the inbox, recovered all three pre-existing keys (`NEXB-398`, `NEXB-442`, `NEXB-443`), took `NEXB-443` as the latest registered one, and generated a correct `NEXB-444`–`NEXB-451` sequence for the eight bugs that lacked one. But it never actually opened the RC screenshots to check whether each Jira "TO DO" item still held. It treated the board's column as the final word and marked 10 of the 11 bugs unsolved — missing all six additional resolved bugs (`Full refund renders complete`, VAT tax, partial balance update, refund-reason validation, the confirmation screen, and duplicate-refund) and reporting only the one bug (Safari login) that the prompt itself had already told it was solved. Ten of the eleven status criteria go uncredited, including the four the model happens to label correctly: those criteria require the call to rest on the screenshot's evidence, and here it rests on a stale board that merely agrees.
...
- But it marked `Is resolved?` → `No` for six bugs that RC evidence shows are actually resolved, and → `Yes` for only Safari.
...
- The four bugs that really are still broken (`Partial refund timeout`, `No refund cap`, `Refund mobile view misrendered`, `Fake refund success`) do end up labeled `No`, which makes the deliverables look partly right — but the label is inherited from the Jira board, not read off the RC screenshot. The HTTP 500 stack trace, the -$76.05 overshoot, the mobile clipping, and the Completed-vs-DECLINED contradiction never appear in the reasoning, so those criteria go uncredited too. A right answer reached without the evidence is exactly what the trajectory-level rubrics exist to catch.
...
- A mid weight (+3) covers the issue-key reasoning step (rubric 23): recovering all three keys that already exist from the email — `NEXB-398`, `NEXB-442`, `NEXB-443` — and identifying `NEXB-443` as the latest, so the generated sequence starts in the right place. It's an Agent Behavior / Trajectory criterion, since it's a reasoning act, not a restated value. Safari's `NEXB-398` sits inside it rather than in its own rubric: it's a second, separate email reference that a model can miss while getting the other two right, which is exactly what happened in the run above.
- Another +3 goes to the three `svg_format.txt` rules (Instruction Following / Final answer/artifact) and to the Slack update (Instruction Following / State change). The format rules are handed to the model verbatim before it acts, so there is no interpretation to make; the Slack post is the only state change to a shared team surface, and it also constrains naming and demands a specific follow-up ask.
- `MEMORY.md` is split so that membership and key mapping are graded apart: listing the right bugs under `SOLVED`/`UNSOLVED` (+1 each) restates conclusions already credited upstream, while labeling each with the correct key is its own result — +3 on the solved side, where two recovered keys must land on the right two bugs across seven entries, and +1 on the unsolved side, where only `NEXB-442` is at stake across four.
- Light weights (+1) otherwise cover the mechanical restatements of an already-determined status inside the SVG (Task Completion, since these just carry a conclusion into the right place).
- There is a single negative, aimed at the one failure mode actually worth penalizing here: addressing bugs outside Rebecca's own Jira notes (-5, Agent Behavior / Trajectory). It targets an explicit prompt boundary — "Do not address bugs not logged in my jira" — and carries the same weight as a full status determination, because inventing a twelfth bug corrupts all four deliverables at once. Nothing else in the set penalizes a choice the prompt and `svg_format.txt` never constrained; elements the task never asked for aren't in scope for grading, however imperfect they look in a given run.
