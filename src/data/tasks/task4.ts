...
Finally, send an update in our qa team slack channel stating the bugs solved and unsolved (without its issue keys, just my jira names) and ask Kai and Amara for help to follow up on the unresolved ones. Also, send the .svg to Colton via email with the subject "current refund module QA status".`,
  ...
  gtfaIssues: {
    ...
    issues: [
      ...
      { key: "NEXB-452", name: "Login timeout mobile Safari", resolved: true, evidence: "rc-refund-build-11.png", reason: "Signed in / Secure session established on Safari (iOS)." },
    ],
  },
...
def test_memory_md_entries_have_issue_keys():
  ...
  rubrics: [
    ...
    { n: 25, text: "The model determines from the user's email that the latest registered issue key is `NEXB-443`, so the newly generated issue keys start at `NEXB-444` and continue through `NEXB-452`, both included, to cover the 9 bugs that do not already have existing issue keys, excluding `NEXB-442` and `NEXB-443`.", points: 3, category: "Agent Behavior — Trajectory", evalTarget: "Trajectory", enforces: "Prompt: 'generate them continuing the count from the latest registered one.'", status: "present", observed: "The model correctly matched NEXB-442 and NEXB-443 from the tracker email and generated NEXB-444 through NEXB-452 for the remaining nine bugs, including Safari.", rationale: "A real reconciliation step across two ticket systems and a correct sequential continuation; weighted +3, independent of whether the status determinations above it were right." },
    { n: 26, text: "The model identifies and labels the `Safari mobile login timeout - Kai tackled` bug with a valid issue key continuing the generated `NEXB-444` to `NEXB-452` sequence established in rubric 25 (e.g. `NEXB-452`).", points: 1, category: "Task Completion — Final answer/artifact", evalTarget: "Final answer/artifact", enforces: "GTFA: issue-key mapping — Safari is the ninth bug covered by the generated range, not a pre-existing key.", status: "present", observed: "MEMORY.md labels Safari 'Safari mobile login timeout - Kai tackled (NEXB-452)', the last key in the generated sequence.", rationale: "Isolates the Safari key specifically: unlike the other 8 generated bugs, Safari has no RC-evidence status miss riding on it (rubric 11 already got it right), so this rubric checks the labeling step in isolation; weighted +1 like the other per-bug key checks." },
  ...
  actualRun: {
    ...
    observations: [
      ...
      },
      {
        ...
        outcome: "pass",
        rubrics: [19, 20, 21, 22, 24, 25, 26],
        expected: "NEXB-442/443 recovered from the tracker email; NEXB-444–452 generated sequentially for the rest, including Safari.",
        what: "The model correctly matched NEXB-442 and NEXB-443 from the tracker email, determined NEXB-443 as the latest registered key, and generated NEXB-444 through NEXB-452 for the remaining nine bugs — Safari included, as the last key in the sequence rather than a separately-known one.",
        evidence: "Trajectory: 'Matched from email notifications: KAN-3 → NEXB-442, KAN-4 → NEXB-443. Generated (continuing from NEXB-443): KAN-1 → NEXB-444, ..., KAN-13 → NEXB-452'.",
      },
      {
