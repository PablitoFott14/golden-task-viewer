...
/* ---- visual language: one treatment per alignment kind ---- */
    ...
    <section className="relative overflow-hidden border-b border-ink-200/70 bg-brand-950">
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-white/70">
          The expectations below are in force now, and every submission is reviewed against them. Start with the
          latest update; earlier ones stay one click away in the log.
        </p>
        ...
        return (
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block truncate text-[12.5px] font-bold leading-tight text-ink-900 lg:whitespace-normal">
                {i + 1}. {t.title}
              </span>
...
/* ---- the topic article: where the reader's attention should land ---- */
  ...
  return (
    <article className="card overflow-hidden">
      ...
      <div className="p-6 sm:p-7">
        ...
        <div className="prose-task mt-6 max-w-none border-t border-ink-100 pt-5">
...
/* ---- browsing one update: rail + article + compact search ---- */
...
function TopicBrowser({ update }: { update: AlignmentUpdate }) {
  ...
  const matches = useMemo(() => {
    ...
    return update.topics.filter(
      (t) => has(t.title) || has(t.impact) || has(t.summary) || has(t.tag) || has(t.body)
    );
  }, [q, update]);
  ...
  return (
    <div>
      {/* Compact search, right-aligned above the content */}
      <div className="mb-5 flex items-center justify-end">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            placeholder="Search this update…"
      ...
      {searching ? (
        ...
        <div className="lg:grid lg:grid-cols-[280px_1fr] lg:items-start lg:gap-6">
          <nav className="mb-5 lg:sticky lg:top-20 lg:mb-0">
            ...
            <TopicRail topics={update.topics} activeId={activeTopic.id} onSelect={setActiveId} />
          </nav>
          <motion.div
            key={activeTopic.id}
...
function UpdatesLog({ activeId, onSelect }: { activeId: string; onSelect: (id: string) => void }) {
  const entries: TimelineEntry[] = alignmentUpdates.map((u) => ({
    id: u.id,
    date: u.dateLabel,
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
      <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-ink-400">
        <History size={13} /> Updates log
      </span>
      <UpdateTimeline entries={entries} activeId={activeId} onSelect={onSelect} />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* Selected update: what actually changed */}
        <motion.div
          key={update.id}
